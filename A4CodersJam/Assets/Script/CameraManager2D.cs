...
public class CameraManager2D : MonoBehaviour
{
    ...
    private bool _isBoundsEnabled = false;
    ...
    private void LateUpdate()
    {
        ...
        {
            _camera.orthographicSize = Mathf.Lerp(_camera.orthographicSize, 40, 0.02f);
            _camera.transform.position = new Vector3(Mathf.Lerp(_camera.transform.position.x, 10, 0.02f), Mathf.Lerp(_camera.transform.position.y, -5, 0.02f), _camera.transform.position.z);
        }
    }
    #region Functions Bounds
    public void SetBound(CameraBounds bounds)
    {
